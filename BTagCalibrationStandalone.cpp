...
BTagEntrySA::Parameters::Parameters(
  OperatingPoint op,
...
BTagEntrySA::BTagEntrySA(const std::string &csvLine)
{
  // make tokens
  ...
  while (std::getline(buff, token, ","[0])) {
    token = BTagEntrySA::trimStr(token);
    if (token.empty()) {
      continue;
    }
    vec.push_back(token);
  ...
  if (vec.size() != 11) {
std::cerr << "ERROR in BTagCalibrationSA: "
...
throw std::exception();
  ...
  params = BTagEntrySA::Parameters(
    BTagEntrySA::OperatingPoint(op),
    vec[1],
    vec[2],
...
BTagEntrySA::BTagEntrySA(const std::string &func, BTagEntrySA::Parameters p):
  formula(func),
  params(p)
{
  TF1 f1("", formula.c_str());  // compile formula to check validity
...
BTagEntrySA::BTagEntrySA(const TF1* func, BTagEntrySA::Parameters p):
...
BTagEntrySA::BTagEntrySA(const TH1* hist, BTagEntrySA::Parameters p):
  params(p)
{
  int nbins = hist->GetNbinsX();
  TAxis const* axis = hist->GetXaxis();
  ...
  if (params.operatingPoint == BTagEntrySA::OP_RESHAPING) {
...
std::string BTagEntrySA::makeCSVHeader()
{
...
std::string BTagEntrySA::makeCSVLine() const
{
  std::stringstream buff;
...
std::string BTagEntrySA::trimStr(std::string str) {
...
BTagCalibrationSA::BTagCalibrationSA(const std::string &taggr):
  tagger_(taggr)
{}
...
void BTagCalibrationSA::addEntry(const BTagEntrySA &entry)
{
  data_[token(entry.params)].push_back(entry);
}

const std::vector<BTagEntrySA>& BTagCalibrationSA::getEntries(
  const BTagEntrySA::Parameters &par) const
{
...
void BTagCalibrationSA::readCSV(const std::string &s)
{
  std::stringstream buff(s);
  readCSV(buff);
}

void BTagCalibrationSA::readCSV(std::istream &s)
{
  std::string line;
  ...
  if (line.find("OperatingPoint") == std::string::npos) {
    addEntry(BTagEntrySA(line));
  }

  while (getline(s,line)) {
    line = BTagEntrySA::trimStr(line);
...
void BTagCalibrationSA::makeCSV(std::ostream &s) const
{
  s << tagger_ << ";" << BTagEntrySA::makeCSVHeader();
  for (std::map<std::string, std::vector<BTagEntrySA> >::const_iterator i
           = data_.cbegin(); i != data_.cend(); ++i) {
    const std::vector<BTagEntrySA> &vec = i->second;
    for (std::vector<BTagEntrySA>::const_iterator j
...
std::string BTagCalibrationSA::makeCSV() const
{
  std::stringstream buff;
  makeCSV(buff);
  return buff.str();
...
std::string BTagCalibrationSA::token(const BTagEntrySA::Parameters &par)
{
  std::stringstream buff;
...
class BTagCalibrationReaderSA::BTagCalibrationReaderImplSA
{
  friend class BTagCalibrationReaderSA;
...
private:
  BTagCalibrationReaderImplSA(BTagEntrySA::OperatingPoint op,
  ...
  BTagEntrySA::OperatingPoint op_;
  std::string sysType_;
  std::vector<std::vector<TmpEntry> > tmpData_;  // first index: jetFlavor
  std::vector<bool> useAbsEta_;                  // first index: jetFlavor
  std::map<std::string, std::shared_ptr<BTagCalibrationReaderImplSA>> otherSysTypeReaders_;
};


BTagCalibrationReaderSA::BTagCalibrationReaderImplSA::BTagCalibrationReaderImplSA(
                                             BTagEntrySA::OperatingPoint op,
                                             const std::string & sysType,
                                             const std::vector<std::string> & otherSysTypes):
  op_(op),
...
throw std::exception();
    ...
    otherSysTypeReaders_[ost] = std::auto_ptr<BTagCalibrationReaderImplSA>(
        new BTagCalibrationReaderImplSA(op, ost)
    );
  }
}

void BTagCalibrationReaderSA::BTagCalibrationReaderImplSA::load(
                                             const BTagCalibrationSA & c,
                                             BTagEntrySA::JetFlavor jf,
                                             std::string measurementType)
{
  if (tmpData_[jf].size()) {
...
throw std::exception();
  ...
  BTagEntrySA::Parameters params(op_, measurementType, sysType_);
  const std::vector<BTagEntrySA> &entries = c.getEntries(params);
  ...
  for (const auto &be : entries) {
    ...
    if (op_ == BTagEntrySA::OP_RESHAPING) {
...
double BTagCalibrationReaderSA::BTagCalibrationReaderImplSA::eval(
                                             BTagEntrySA::JetFlavor jf,
                                             float eta,
                                             float pt,
                                             float discr) const
{
  bool use_discr = (op_ == BTagEntrySA::OP_RESHAPING);
...
double BTagCalibrationReaderSA::BTagCalibrationReaderImplSA::eval_auto_bounds(
...
std::pair<float, float> BTagCalibrationReaderSA::BTagCalibrationReaderImplSA::min_max_pt(
                                               BTagEntrySA::JetFlavor jf,
                                               float eta,
                                               float discr) const
{
...
BTagCalibrationReaderSA::BTagCalibrationReaderSA(BTagEntrySA::OperatingPoint op,
                                             const std::string & sysType,
                                             const std::vector<std::string> & otherSysTypes):
  pimpl(new BTagCalibrationReaderImplSA(op, sysType, otherSysTypes)) {}

void BTagCalibrationReaderSA::load(const BTagCalibrationSA & c,
                                 BTagEntrySA::JetFlavor jf,
...
double BTagCalibrationReaderSA::eval(BTagEntrySA::JetFlavor jf,
                                   float eta,
                                   float pt,
                                   float discr) const
{
...
double BTagCalibrationReaderSA::eval_auto_bounds(const std::string & sys,
                                               BTagEntrySA::JetFlavor jf,
                                               float eta,
                                               float pt,
                                               float discr) const
...
std::pair<float, float> BTagCalibrationReaderSA::min_max_pt(BTagEntrySA::JetFlavor jf,
                                                          float eta,
                                                          float discr) const
{
